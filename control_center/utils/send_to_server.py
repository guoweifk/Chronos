...
def send_to_server(agent_ip: str, message_obj):
    """
    发送消息到 agent，并尝试读取对方返回的 JSON 结果。
    ...
    try:
        ...
        if not chunks:
            logging.info(f"[✓] 成功发送消息到 agent {agent_ip}，但对方未返回数据")
            return True, None

        raw_resp = b"".join(chunks).decode("utf-8", errors="replace")
        logging.debug(f"[←] 从 agent {agent_ip} 收到响应: {raw_resp}")
        ...
        logging.error(f"[×] 发送消息到 agent {agent_ip} 失败: {e}")
        return False, None
