...
def ProcessMenu(PDU, client, session_dict, msg):
    if msg == "Q\n" or msg == "q\n": 
        ...
        #S1SetupRequest 函数：根据 session_dict 中的会话信息（如 S1-TYPE、ENB-ID 等），构建一个包含多种信息元素的 S1 Setup Request 消息。

        #发送消息：在主程序中，当用户选择菜单项 15 时，调用 S1SetupRequest(session_dict) 构建该消息，并将其编码为 APER 格式后，通过 client.send(message) 发送给目标服务器。

        #通过调用 S1SetupRequest(session_dict) 函数，使用 session_dict 中存储的会话数据构建一个 S1 Setup Request 消息。
        PDU.set_val(S1SetupRequest(session_dict))
        # PDU.to_aper() 将构建好的消息编码为 APER（ASN.1 PER Encoding Rules）格式
        message = PDU.to_aper()
        client = set_stream(client, 0)
        # 通过 client.send(message) 将生成的消息通过 SCTP 套接字发送给指定的目标。set_stream(client, 0) 会设置一个网络流
        ...
        if session_dict['STATE'] >0:      
            ...
            session_dict['SQN'] = 0
            session_dict['ENB-UE-S1AP-ID'] = random.randrange(10000) # 这个可能是需要修改的嗷
            PDU.set_val(InitialUEMessage(session_dict))
            message = PDU.to_aper()
            client = set_stream(client, 1)
            bytes_sent = client.send(message)    
...
def print_log(session_dict, log_message):
    ...
    # print_menu(session_dict['LOG'])
    # session_dict 里面有个LOG字段，这里就是添加这个log
    # print(session_dict['LOG'][-1])
    # with open('log.txt', 'a') as log_file:
    #     log_file.write(session_dict['LOG'][-1] + '\n')
    return session_dict    
