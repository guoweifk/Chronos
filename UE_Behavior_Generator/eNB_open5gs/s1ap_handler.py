...
from pycrate_asn1dir import S1AP  # 保留原依赖，便于你内部使用
from eNB_LOCAL import *           # 你现有代码在 _process_message_for_ue 里引用
from myutils import hexdump       # 如不需要可移除
...
class MultiThreadedS1APHandler:
    ...
    def __init__(
        ...
        self.stats = {
            ...
            'errors': 0,
            'queue_full': 0,     # 放入失败（满）
            'dropped': 0         # 主动丢弃（如 put_nowait 失败）
        }
        ...
        # 执行器
        # 1) 计算/处理 worker 池（固定并发）
        self.executor = ThreadPoolExecutor(
            max_workers=max_workers,
            thread_name_prefix="S1AP-Worker"
        )
        # 2) 文件写入单线程 I/O 池（避免被计算任务挤压）
        self.io_executor = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="S1AP-IO"
        )

        # 日志：去掉热路径 print，用 logging 写到文件（stats.txt 由 _stats_loop 负责）
        self._init_logging()
    ...
    def extract_enb_ue_s1ap_id(buffer):
        try:
            ...
            if buffer.startswith(b'\x00\x0b'):  # DownlinkNASTransport
                if len(buffer) > 34:
