...
class BaseStationConfig:
    ...
    ues_per_second: int
    imsi_base: str
    udp_port: int = 2152  # 新增：每个基站的UDP监听端口
...
class BaseStationManager:
    ...
    def _run_base_station(config: BaseStationConfig):
        ...
        # 创建并启动eNB
        enb = ENB(config.enb_name, options_list[0])
        enb.connect_to_mme()
        enb.s1_setup()
        
        print(f"[{config.enb_name}] 基站启动完成，IP: {config.enb_ip}")
...
def create_configs() -> List[BaseStationConfig]:
    ...
    return [
        BaseStationConfig(
            enb_id="enb_001",
            enb_name="ZH1-eNB", 
            enb_ip="192.168.55.110",
            mme_ip="192.168.55.137",
            num_ues=1,
            ues_per_second=1,
            imsi_base="466920000000001"
        )
        # BaseStationConfig(
        #     enb_id="enb_002",
        #     enb_name="ZH2-eNB",
