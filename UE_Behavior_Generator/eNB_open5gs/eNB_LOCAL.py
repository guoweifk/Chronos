...
def S1SetupRequest(dic):
    ...
    if dic['S1-TYPE'] == "4G" :
        ...
        # IEs.append({'id': 64, 'value': ('SupportedTAs', [{'tAC': dic['ENB-TAC1'], 'broadcastPLMNs': [dic['ENB-PLMN']]},
        #                                                  ]),
        #             'criticality': 'reject'})
...
#### ESM #####
        ...
        for i in nas_list:
            ...
            elif i[0] == 'protocol configuration options':
                # print("------------------这里应该打印P-CSCF的IP地址-------------------")
                # print(i[1])
                pco_bytes = i[1]  # 获取原始字节数据
                # 打印原始字节数据（用于调试）
                # print("Raw PCO bytes:", pco_bytes.hex())
                # 尝试解析 P-CSCF IPv4 地址
                try:
                    # 查找 P-CSCF IPv4 地址容器 (ID = 0x000c)
                    # 容器格式: [ID高字节, ID低字节, 长度, 数据...]
                    # 查找模式: \x00\x0c\x04 后跟4字节IP地址
                    # 查找 P-CSCF 容器标识符
                    p_cscf_index = pco_bytes.find(b'\x00\x0c\x04')
                    if p_cscf_index != -1 and len(pco_bytes) >= p_cscf_index + 7:
                        # 提取IP地址字节 (4字节)
                        ip_bytes = pco_bytes[p_cscf_index+3:p_cscf_index+7]
                        # 将字节转换为IPv4地址
                        p_cscf_ip = ".".join(str(byte) for byte in ip_bytes)
                        print(f"P-CSCF IPv4 Address: {p_cscf_ip}")
                        dic['P-CSCF-IPv4'] = p_cscf_ip
                    else:
                        print("P-CSCF IPv4 Address container not found")
                except Exception as e:
                    print(f"Error parsing P-CSCF address: {str(e)}")
...
def ProcessS1AP(PDU, client, session_dict, client_number, buffer=None):
    
    """
    处理S1AP消息
    :param buffer: 可选参数，直接传入的消息数据
    """
    # 如果有传入的buffer，使用它而不是recv
...
#     (type, pdu_dict) = PDU()
    ...
    if type == 'initiatingMessage':
        ...
        #UE Related:
            ...
            # print("--------------answer_list--------------------------")
            # print(f"answer_list 长度: {len(answer_list)}")  # 添加这行调试
            # print(f"answer_list 内容: {answer_list}")      # 添加这行调试
            ...
            for answer in answer_list:
                if answer != None:
                    ...
                    message = PDU.to_aper()               
                    bytes_sent = client.send(message)
            time.sleep(0.3)  # 这个0.2是猜测连续两条消息的间隔事件为0.2s。暂不确定
